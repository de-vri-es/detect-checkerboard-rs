...
fn do_main(options: Options) -> Result<(), String> {
	...
	let mut greyscale = image.to_luma8();
	imageproc::contrast::equalize_histogram_mut(&mut greyscale);
	...
	let binarized = imageproc::contrast::threshold(&greyscale, 40);
	...
	let mut visual: image::RgbImage = (&greyscale).convert();
	for (contour, quad) in contours.iter().zip(quads.iter()) {
		for (a, b) in contour.points.iter().zip(contour.points.iter().skip(1)) {
			let a = (a.x, a.y);
			let b = (b.x, b.y);
			imageproc::drawing::draw_antialiased_line_segment_mut(&mut visual, a, b, magenta, imageproc::pixelops::interpolate);
		}
		...
		for (a, b) in quad.iter().zip(quad.iter().cycle().skip(1)) {
			let a = (a.x, a.y);
			let b = (b.x, b.y);
			imageproc::drawing::draw_antialiased_line_segment_mut(&mut visual, a, b, cyan, imageproc::pixelops::interpolate);
		}
	}
